...
const CATEGORY_FILTERS = ["all", "natura", "pensiero", "suono", "corpo", "altro"] as const;
...
const OfferingsApproved = () => {
  ...
  const { data: offerings = [], isLoading } = useQuery<AdminOffering[]>({
    queryKey: ["admin-offerings", "approved", mediaFilter, categoryFilter],
    queryFn: async () => {
      if (!isAdmin) return [];
      let query = supabase
        .from("offerings")
      ...
      let result = (data || []) as AdminOffering[];

      // Category filter — checks title/note for category keywords
      if (categoryFilter !== "all") {
        const keywords: Record<string, string[]> = {
          natura: ["albero", "pianta", "fiore", "mare", "vento", "acqua", "terra", "sole", "luna", "cielo", "pietra", "sasso", "foglia"],
          pensiero: ["idea", "parola", "frase", "poesia", "pensiero", "sogno", "memoria", "racconto"],
          suono: ["canto", "musica", "nota", "suono", "melodia", "armonia", "ritmo", "silenzio"],
          corpo: ["corpo", "mano", "piede", "viso", "cuore", "respiro", "movimento", "gesto"],
        };
        const kw = keywords[categoryFilter] ?? [];
        if (kw.length > 0) {
          result = result.filter((o) => {
            const text = `${o.title ?? ""} ${o.note ?? ""}`.toLowerCase();
            return kw.some((k) => text.includes(k));
          });
        }
      }

      return withSignedFileUrls(result);
    },
    enabled: isAdmin,
  });
...
export default OfferingsApproved;
