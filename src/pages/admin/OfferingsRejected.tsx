...
const OfferingsRejected = () => {
  ...
  const { data: offerings = [], isLoading } = useQuery<AdminOffering[]>({
    queryKey: ["admin-offerings", "rejected", mediaFilter],
    queryFn: async () => {
      if (!isAdmin) return [];
      let query = supabase
        .from("offerings")
  ...
  return (
    <AdminShell rightContent={<AdminThemeToggle mode={mode} onChange={setThemeMode} />}>
      ...
      <section className="mb-6 rounded-2xl border border-border bg-card/70 p-5 shadow-sm backdrop-blur">
        ...
        <h1 className="mt-2 text-3xl leading-tight md:text-4xl">Cavapendolate rifiutate</h1>
        <p className="mt-3 max-w-2xl text-sm text-muted-foreground">
          Le cavapendolate rifiutate non sono state accolte. Puoi riapprovarle se cambi idea.
        </p>
...
export default OfferingsRejected;
