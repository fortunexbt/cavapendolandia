...
const BlockPreview = ({ title, body }: { title?: string | null; body?: string | null }) => (
  <div className="rounded-2xl border border-border bg-card/50 p-4">
    ...
    {!title && !body && <p className="text-sm italic text-muted-foreground/60">Nessun contenuto.</p>}
  </div>
);
...
const PagesEditorStub = () => (
  <div className="flex min-h-[60vh] items-center justify-center">
    <div className="max-w-md rounded-2xl border border-border bg-card/70 p-8 text-center">
      ...
      <h2 className="mt-3 text-2xl font-light tracking-[0.08em]">Pagine</h2>
      <p className="mt-3 text-sm text-muted-foreground">
        L'editor per le pagine CMS è in fase di preparazione. presto disponibile.
      </p>
    </div>
  </div>
);
  ...
  return (
    <AdminShell rightContent={<AdminThemeToggle mode={mode} onChange={setThemeMode} />}>
      <div className="mb-6 rounded-2xl border border-border bg-card/70 p-5 shadow-sm backdrop-blur">
        ...
        <h1 className="mt-2 text-3xl leading-tight md:text-4xl">Editor Pagine</h1>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="space-y-4 rounded-2xl border border-border bg-card/70 p-5 shadow-sm">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Contenuto del blocco..."
            />
          </div>
        ...
        <div className="space-y-4">
          ...
          {isLoading && <p className="text-sm italic text-muted-foreground/70">Carico…</p>}
        </div>
      </div>
    </AdminShell>
  );
