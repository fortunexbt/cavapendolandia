...
const PratoEditorStub = () => (
  <div className="flex min-h-[60vh] items-center justify-center">
    <div className="max-w-md rounded-2xl border border-border bg-card/70 p-8 text-center">
      ...
      <h2 className="mt-3 text-2xl font-light tracking-[0.08em]">Prato</h2>
      <p className="mt-3 text-sm text-muted-foreground">
        L'editor del prato è in fase di preparazione. presto disponibile.
      </p>
    </div>
  </div>
);
  ...
  return (
    <AdminShell rightContent={<AdminThemeToggle mode={mode} onChange={setThemeMode} />}>
      <div className="mb-6 rounded-2xl border border-border bg-card/70 p-5 shadow-sm backdrop-blur">
        ...
        <h1 className="mt-2 text-3xl leading-tight md:text-4xl">Editor Prato</h1>
      </div>
      ...
      <div className="mb-6 rounded-2xl border border-border bg-card/70 p-5 shadow-sm">
        <p className="font-mono-light text-[0.62rem] uppercase tracking-[0.14em] text-muted-foreground mb-4">
          {editingId ? "Modifica elemento" : "Aggiungi elemento"}
        </p>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          >
            ...
            <option value="monolith">Monolite</option>
            <option value="lantern">Lanterma</option>
            <option value="billboard">Cartello</option>
          </select>
          <input
            value={form.label}
      ...
      {isLoading ? (
        <p className="py-8 text-center text-sm text-muted-foreground/70 italic">Carico elementi…</p>
      ) : elements.length === 0 ? (
        <div className="rounded-2xl border border-border bg-card/70 p-12 text-center">
          <p className="text-sm text-muted-foreground/70 italic">Nessun elemento nel prato.</p>
        </div>
      ) : (
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="font-mono-light text-[0.62rem] uppercase tracking-[0.1em] text-muted-foreground">
                      {el.element_type} · x: {el.position_x} · z: {el.position_z} · scala: {el.scale} · rot: {el.rotation}
                    </p>
                  </div>
                </div>
