...
const MYSTICAL_OPENING_PARTS = [
  "index.mysticalOpening1",
  "index.mysticalOpening2",
  "index.mysticalOpening3",
  "index.mysticalOpening4",
  "index.mysticalOpening5",
  "index.mysticalOpening6",
  "index.mysticalOpening7",
  "index.mysticalOpening8",
  "index.mysticalOpening9",
  "index.mysticalOpening10",
  "index.mysticalOpening11",
  "index.mysticalOpening12",
  "index.mysticalOpening13",
];
...
const Index = () => {
  const { t } = useTranslation();
  const { getBlock } = usePageBlocks("index");
        ...
        <div className="relative w-full overflow-hidden rounded-[2rem] border border-foreground/10 bg-background/58 px-6 py-8 shadow-[0_40px_120px_rgba(73,48,29,0.16)] backdrop-blur-xl md:px-12 md:py-12">
          ...
          <motion.div variants={itemVariants} className="relative mt-8">
            ...
            <p className="px-3 font-serif text-lg italic leading-[1.82] text-foreground/82 md:px-8 md:text-[1.18rem]">
              {MYSTICAL_OPENING_PARTS.map((key, index) => (
          ...
          >
            {[
              { to: "/galleria", label: t("index.navGallery") },
              { to: "/offri", label: t("index.navOffri") },
              { to: "/regole", label: t("index.navRegole") },
            ].map((item) => (
              <Link
                key={item.to}
                to={item.to}
